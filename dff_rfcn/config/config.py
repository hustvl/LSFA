...
config.network.rnet_num_conv = None
config.network.fnet_type = None
config.network.fuse_type = 'add' # 'concat'
config.network.res_diff_bn = False
config.network.add_dcn = False
config.network.add_small_net = False
config.network.small_net_bn_before_fuse = False
config.network.small_net_scale_before_fuse = False
config.network.small_net_stride = 4 # or 8
config.network.small_net_fuse_type = 'add' # or 'concat'
config.network.add_Nq_net = False
config.network.add_Fgfa_net = False
...
# DFF
config.TEST.KEY_FRAME_INTERVAL = 12
...
def update_network_config():
    if 'resnet' in config.network.pretrained:
        config.network.PIXEL_MEANS = [0, 0, 0]
        config.network.PIXEL_SCALE = 1.0
        nettype, num_layer = config.network.pretrained.split('-')
        config.network.num_layer = int(float(num_layer))
        config.network.nettype = 'resnet'
    elif 'mobilenetv2-1_0-hobot' in config.network.pretrained:
        config.network.PIXEL_MEANS = [103.94, 116.78, 123.68]
        config.network.PIXEL_SCALE = 0.017
        config.network.nettype = 'mobilenet_hobot'
    elif 'mobilenetv2-1_0' in config.network.pretrained:
        config.network.PIXEL_MEANS = [103.94, 116.78, 123.68]
        config.network.PIXEL_SCALE = 1.0
        config.network.nettype = 'mobilenet'
    else:
        raise RuntimeError("unknow nettype")
