...
class TestLoader(mx.io.DataIter):
    def __init__(self, roidb, config, batch_size=1, shuffle=False,
        ...
        self.data_name = ['data', 'im_info', 'data_key', 'data_key_old', 'motion_vector', 'res_diff', 'feat_key_old', 'feat_key']
    ...
    def get_batch(self):
        ...
        data, label, im_info = get_rpn_testbatch([cur_roidb], self.cfg, self.cur_frameid)
        if self.key_frameid == self.cur_frameid: # key frame
            self.data_key_old = self.data_key.copy() if self.data_key is not None else data[0]['data'].copy()
        ...
        extend_data = [{'data': data[0]['data'],
                        ...
                        'data_key_old': self.data_key_old,
                        'motion_vector': data[0]['motion_vector'],
                        'res_diff': data[0]['res_diff'],
                        'feat_key_old': np.zeros((1,self.cfg.network.DFF_FEAT_DIM,1,1)),
...
class AnchorLoader(mx.io.DataIter):
    ...
    def __init__(self, feat_sym, roidb, cfg, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
        ...
        if config.TRAIN.END2END:
            self.data_name = ['data_ref_old', 'data', 'data_ref', 'eq_flag', 'eq_flag_old', 'im_info', 'gt_boxes', 'motion_vector', 'res_diff']
        else:
            self.data_name = ['data']
