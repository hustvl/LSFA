...
from utils.PrefetchingIter import PrefetchingIter, MultiThreadPrefetchingIter
...
def im_detect(predictor, data_batch, data_names, scales, cfg):
    ...
    if output_all[0].has_key('choose_feat_output'):
        feat = output_all[0]['choose_feat_output']
    else:
        feat = None
...
def pred_eval(gpu_id, key_predictor, cur_predictor, test_data, imdb, cfg, vis=False, thresh=1e-4, logger=None, ignore_cache=True):
    ...
    #if not isinstance(test_data, PrefetchingIter):
    #    test_data = PrefetchingIter(test_data)
    if not isinstance(test_data, MultiThreadPrefetchingIter):
        test_data = MultiThreadPrefetchingIter(test_data, test_phase=True)
    ...
    for im_info, key_frame_flag, data_batch in test_data:
        ...
        if key_frame_flag != 2:
            if key_frame_flag == 0:
                feat_old = mx.nd.zeros((1,1024,1,1))
            else:
                feat_old = feat.copy()
            data_batch.data[0][-2] = feat_old     
            data_batch.provide_data[0][-2] = ('feat_key_old', feat_old.shape)
