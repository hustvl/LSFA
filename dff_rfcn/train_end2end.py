...
import os
import sys
from config.config import config, update_config, update_network_config
...
from utils.PrefetchingIter import MultiThreadPrefetchingIter
...
def train_net(args, ctx, pretrained, pretrained_flow, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    ...
    max_data_shape = [('data', (config.TRAIN.BATCH_IMAGES, 3, max([v[0] for v in config.SCALES]), max([v[1] for v in config.SCALES]))),
                      ('data_ref_old', (config.TRAIN.BATCH_IMAGES, 3, max([v[0] for v in config.SCALES]), max([v[1] for v in config.SCALES]))),
                      ('data_ref', (config.TRAIN.BATCH_IMAGES, 3, max([v[0] for v in config.SCALES]), max([v[1] for v in config.SCALES]))),
                      ('motion_vector', (config.TRAIN.BATCH_IMAGES, 2, 38, 63)),
                      ('res_diff', (config.TRAIN.BATCH_IMAGES, 3, 38, 63)),
                      ('eq_flag_old', (1,)), 
                      ('eq_flag', (1,))]
    ...
    # create solver
    fixed_param_prefix = None #config.network.FIXED_PARAMS
    ...
    if not isinstance(train_data, MultiThreadPrefetchingIter):
        train_data = MultiThreadPrefetchingIter(train_data)
