...
def get_predictor(sym, sym_instance, cfg, arg_params, aux_params, test_data, ctx):
    ...
    max_data_shape = [[('data', (1, 3, max([v[0] for v in cfg.SCALES]), max([v[1] for v in cfg.SCALES]))),
                      ('motion_vector', (1, 2, 38, 63)),
                      ('res_diff', (1, 3, 38, 63)),
                      ('data_key', (1, 3, max([v[0] for v in cfg.SCALES]), max([v[1] for v in cfg.SCALES]))),
                      ('data_key_old', (1, 3, max([v[0] for v in cfg.SCALES]), max([v[1] for v in cfg.SCALES]))),]]
...
def test_rcnn(cfg, dataset, image_set, root_path, dataset_path,
    ...
    key_sym_instance = eval(cfg.symbol + '.' + cfg.symbol)(cfg)
    cur_sym_instance = eval(cfg.symbol + '.' + cfg.symbol)(cfg)
