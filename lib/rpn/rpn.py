...
def get_rpn_pair_batch(roidb, cfg):
    ...
    """
    assert len(roidb) == 1, 'Single batch only'
    imgs, ref_imgs, old_ref_imgs, eq_flags, eq_flags_old, roidb, motion_vector, res_diff = get_pair_image(roidb, cfg)
    im_array = imgs[0]
    ref_im_array = ref_imgs[0]
    old_ref_im_array = old_ref_imgs[0]
    motion_vector_array = motion_vector[0]
    res_diff_array = res_diff[0]
    eq_flag_array = np.array([eq_flags[0],], dtype=np.float32)
    eq_flag_old_array = np.array([eq_flags_old[0],], dtype=np.float32)
    im_info = np.array([roidb[0]['im_info']], dtype=np.float32)
    ...
    data = {'data': im_array,
            ...
            'data_ref_old': old_ref_im_array,
            'eq_flag': eq_flag_array,
            'eq_flag_old': eq_flag_old_array,
            'motion_vector': motion_vector_array,
            'res_diff': res_diff_array,
            'im_info': im_info}
    label = {'gt_boxes': gt_boxes}
